...
# Reliable dataset loading for local + Streamlit Cloud
...
def build_engine():
    ...
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(
            f"Dataset not found: {DATA_PATH}. Keep all_job_post.csv in the same folder as app.py"
        )

    if os.path.getsize(DATA_PATH) == 0:
        raise ValueError("all_job_post.csv is empty (0 bytes).")

    try:
        df = pd.read_csv(DATA_PATH, encoding="utf-8")
    except pd.errors.EmptyDataError:
        raise ValueError(
            "CSV contains no readable columns. Check whether the file is empty or corrupted."
        )
